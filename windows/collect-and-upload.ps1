# === AI Sync — 완전 무소음 백그라운드 서비스 ===
# 창 절대 안 뜸. Write-Host 없음. schtasks 재등록 없음.
...
$sessions = @()
$logFile = "$env:USERPROFILE\.ai-sync-hub\sync.log"
...
if (-not (Test-Path $tokenFile)) { Log "No token file"; exit 0 }
$token = [IO.File]::ReadAllText($tokenFile).Trim()
if (-not $token) { Log "Empty token"; exit 0 }
...
try {
    ...
    Log "OK: $($sessions.Count) sessions synced"
} catch {
    Log "FAIL: $_"
}
